        ...
        if total_dim[i][1] < max_dim:
            ...
            if diff % 2 == 0:
                padded_input[i] = ZeroPadding2D(
                    padding=(half_diff, half_diff)
                )(inputs[i])
            else:
                padded_input[i] = ZeroPadding2D(
                    padding=(
                        (half_diff, half_diff + 1),
                        (half_diff, half_diff + 1),
                    )
                )(inputs[i])
        else:
...
def reduction(x, config):
    ...
    if dim[1] == 1 or dim[2] == 1:
        print(
            "WARNING: One or more dimensions of the input of the reduction layer is 1. It cannot be further reduced. "
    ...
    if parameters["reduction_type"] == "max_pooling":
        ...
        raise ValueError(
            f'Unknown reduction type in config: {parameters["reduction_type"]}'
        )
