...
def build_dataset(dataset_file_path, databag):
    dataset = load_dataset(dataset_file_path)

    if databag["dataset_type"] == DatabagType.zip_file:
        download_and_extract_zip_file(
            databag["bucket_name"], databag["file_name"]
        )
        dataset["file"] = dataset["file"].map(path_to_absolute)
    return dataset


def download_and_extract_zip_file(bucket_name, file_name):
    zip_file = "dataset.zip"
    download_file_from_bucket(bucket_name, file_name, zip_file)
