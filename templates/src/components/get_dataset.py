...
from objectstore.objectstore import error_databag_status_update, get_download_url, download_file
    ...
    with exception_handler(handler, ErrorMsgKey.DATASET_COULD_NOT_BE_LOADED):
        with open(dataset.path, 'wb') as output_file:
            if dataset_type == DatasetType.file_url:
                ...
                download_url = get_download_url(bucket, f"{databag_id}/{file_name}", os4ml_namespace)
