...
export function visitDatabagPage(): void {
  cy.visit('/#/databags');
  cy.get('[data-testid="databags-page"]', {timeout: TIMEOUT_LONG}).should('be.visible');
}
export function createDatabag(
  databagName: string,
  selectFile: string,
) {
  cy.get('[data-testid="add-databag"]', { timeout: TIMEOUT_SHORT }).first().click();
  ...
  cy.get('[data-testid="finish-upload"]', { timeout: TIMEOUT_LONG }).should('not.be.disabled').click();
}
export function checkDatabag(
  databagName: string,
) {
...
export function changeDatabagName(databagName: string, updatedDatabagName: string) {
...
export function deleteDatabag(databagName: string) {
  cy.get('[data-testid="databag-item"]',{
    ...
    .find('[data-testid="databag-settings-button"]')
    .click();
  cy.get('[data-testid="databag-delete-button"]', { timeout: TIMEOUT_SHORT }).click();
  cy.get('[data-testid="confirm-popup-button"]', { timeout: TIMEOUT_SHORT }).click();
  cy.get('[data-testid="databags-page"]').should('not.contain', databagName, { timeout: TIMEOUT_SHORT });
}
export function visitSolutionPage(): void {
  cy.visit('/#/solutions');
  cy.get('[data-testid="solutions-page"]', {timeout: 600000}).should('be.visible');
}
export function setupSolutionTestDatabag(solutionTestDatabagName: string):void {
  visitDatabagPage()
  cy.get('[data-testid="databag-item"]').should('exist').then($items => {
...
export function createSolution(solutionName: string, databagName: string) {
  cy.get('[data-testid="add-solution"]', { timeout: TIMEOUT_SHORT }).first().click();
  cy.checkA11y(undefined, undefined, handleA11yViolations, true);
  cy.get('[data-testid="input-name"]', { timeout: TIMEOUT_SHORT }).type(solutionName);
  cy.get('[data-testid="input-databagId"]', { timeout: TIMEOUT_SHORT }).should('not.be.disabled').click();
  cy.get('mat-option').contains(databagName).click();
  cy.get('[data-testid="output-select-field"]', { timeout: TIMEOUT_SHORT }).should('not.be.disabled').click();
...
export function changeSolutionName(solutionName: string, updatedSolutionName: string): void {
  cy.get('[data-testid="solution-item"]')
    .filter(`:contains("${solutionName}")`)
...
export function deleteSolution(solutionName: string) {
  cy.get('[data-testid="solution-item"]')
    ...
    .find('[data-testid="solution-detail-button"]')
    .click();
  cy.get('[data-testid="solution-detail-page"]', {timeout: 600000}).should('be.visible');
  cy.checkA11y(undefined, undefined, handleA11yViolations, true);
